defimpl Inspect, for: Graph do
  ...
  defmacrop is_primitive?(x) do
    quote do: is_binary(unquote(x)) or is_number(unquote(x)) or is_atom(unquote(x))
  end
  ...
  def inspect(%Graph{type: type, vertices: vs, out_edges: es, edges: meta}, opts) when map_size(vs) < 100 do
    # The goal here is to strip out the ids map, convert the vertices map to a list of vertices
    # and convert the map of edges to their reified forms (i.e. the actual vertex term is used in
    # place of ids) we also want to respect the inspect options as much as possible, so we do this
    # all the hard way by constructing the inspect algebra by hand
    vs_doc = Inspect.Algebra.to_doc(Map.values(vs), opts)
    doc = Inspect.Algebra.concat([Inspect.Algebra.empty, "#Graph<type: #{type}, vertices:", " ", vs_doc, ",", " ", "edges: [", ""])
    doc =
      es
      |> Stream.flat_map(fn {v_id, out_neighbors} ->
        ...
        Enum.flat_map(out_neighbors, fn out_id ->
          ...
          Enum.map(Map.fetch!(meta, {v_id, out_id}), fn
            ...
            {nil, _} ->
              [v, " <-> ", out_v_doc]
            {label, _} when type == :directed and is_primitive?(label) ->
              [v, " -[#{label}]-> ", out_v_doc]
            {label, _} when is_primitive?(label) ->
              [v, " <-[#{label}]-> ", out_v_doc]
